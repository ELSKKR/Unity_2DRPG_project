...
- `SignPost_SouthRoad` **不刪**，文字改成前一個旅人留下的字條，並把玩家引向 C 屋，例如：「如果你也繞回來了，別慌。去找住在東邊的人。」（sprite 目前是柵欄立柱佔位圖，換不換由使用者決定）
...
- 玩家走進觸發區（且森林的路還沒開）：
  1. 玩家鎖住移動
  2. **第一次**：用 `RiftCutscene` 播完整演出（至少包含「……回過神來，你又回到原處。」）
  3. **之後**：只跳一行「……回過神來，你又回到原處。」，不播全黑演出
  4. 玩家傳送到該觸發區對應的回位點（預設：往邊界內退約 4 格），不能卡在任何碰撞體裡
- 「第一次」要記在 `WorldStateManager` 的旗標，所以存檔讀檔後維持。
...
- [ ] 每一個回位點：`Physics2D.OverlapCircle(半徑0.4)` 沒有碰到阻擋物，而且位在洪水填充的可達範圍內
- [ ] 第一次進觸發區播完整演出，第二次只跳一行；存檔讀檔後仍然只跳一行
