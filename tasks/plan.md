...
- [x] 洪水填充：邊界封閉，每個觸發區都走得到，每個回位點都安全
- [x] 退出 Play mode 後重開場景，清點關鍵物件都還在
- [x] 使用者實際玩過鬼打牆，確認手感

### Phase 2：角色和對話
- [x] T5 三個新 NPC 的素材、Animator、prefab，並放進場景（對話先放佔位台詞）
- [x] T6 對白稿：六個人的新增和改寫台詞、五筆情報（**使用者審**）
...
### Phase 3：結尾和收尾
- [x] T11 `ending`：「路開了」的判定，以及走／留的選項
- [x] T12 從標題畫面走完整個流程、建置、補寫系統開發歷程
