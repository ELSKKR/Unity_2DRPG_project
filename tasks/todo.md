...
**驗收**
- [ ] 至少 2 種林道做法：一種在現有牆內，一種把牆、相機下限、草地往南推，各自標出林道長度（格數）
- [ ] 魯克的露宿點 2～3 個候選位置
- [ ] 回位點預設往邊界內退 4 格，圖上標示出來
- [ ] 使用者選定
**驗證**：讀回 PNG 確認標示正確
**相依**：無
**檔案**：只有 scratchpad
...
**驗收**
- [ ] `Boundary_Fence` 的 tile 數是 0；`Gate` 和 `Gate_ReadZone` 不存在
- [ ] 洪水填充的可達範圍和改之前一致（x −59～40、y −54～55，林道延伸的部分除外），沒有任何一格在牆外
- [ ] 每個觸發區都至少有一格可達（列出觸發區總數和可達的數量）
- [ ] 每個回位點用 `OverlapCircle(0.4)` 檢查沒有碰到阻擋物，而且位在可達範圍內
